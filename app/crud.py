...
async def get_data(db: Session, datalogger: str, since: Optional[str] = None, before: Optional[str] = None):
    query = db.query(Data).filter(Data.label == datalogger)
    if since:
        query = query.filter(Data.measured_at >= since)
    if before:
        query = query.filter(Data.measured_at <= before)
    return query.all()


# Function to get aggregated data
async def get_aggregated_data(db: Session, datalogger: str, span: str, since: Optional[str] = None,
                              before: Optional[str] = None):
    if span not in ["hour", "day", "month"]:
        raise ValueError("Invalid span value. Must be 'hour', 'day', or 'month'.")

    query = db.query(
        Data.label,
        func.date_trunc(span, Data.measured_at).label('timespan'),
        func.avg(Data.value).label('average_value')
    ).filter(Data.label == datalogger)
    ...
    query = query.group_by(Data.label, 'timespan')
    return query.all()
